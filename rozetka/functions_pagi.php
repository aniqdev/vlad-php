...
?>
 <div class="pagination">
    ...
    <ul>
        ...
        // ceil($total_count / $limit) кол-во страниц
        $button_count = 0;
...
?>
 <div class="bs-pagination">
    <ul class="pagination">
        <?php if($offset > 0): ?>
            <li class='page-item'><a class='page-link' href="<?= $prev_link ?>" class="page-link prev active"><i class="bi bi-arrow-left"></i></a></li>
        <?php else: ?>
        ...
        for ($i=$from; $i < $to; $i++): ?>
            <?php if($current_page == $i): ?>
                <li class='page-item active'><a class='page-link' class="active"><?= $i + 1 ?></a></li>
            <?php else: ?>
                <li class='page-item'><a class='page-link' href="<?= query_add(['offset'=> $i * $limit]) ?>"><?= $i + 1 ?></a></li>
            <?php endif ?>
        <?php endfor;
        echo $right_prefix; ?>
        <?php if($total_count > $offset + $limit): ?>
            <li class='page-item'><a class='page-link' href="<?= $next_link ?>" class="prev active"><i class="bi bi-arrow-right"></i></a></li>
        <?php else: ?>
            <li class='page-item disabled'><a class='page-link' class="prev disabled"><i class="bi bi-arrow-right"></i></a></li>
        <?php endif ?>
    </ul>
</div>
<?php
